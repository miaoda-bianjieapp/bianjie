...
9. `toolId 开发者查重结果`是否记录了开发者实际查询 PostgreSQL 的结果；管理员再检查共享表中是否有尚未入库的并行任务占用同一 ID。
10. `共享任务表记录草稿`是否只有 1 条，并包含本 Issue 的全部工具、toolId、operation、负责人、分支建议和公共文件占用。
11. `共享迁移占用表记录草稿`数量是否等于 Flyway SQL 文件数量：无迁移为 0 条，一个文件为 1 条，多个文件逐条说明依赖顺序。

Issue 中填写“待 AI/管理员确认”是允许的。管理员应做架构判断，而不是要求普通开发者盲填。

批准 Issue 时，管理员应修正草稿中的 Task ID、最终 toolId、executor/operation、个人分支、公共文件占用和数据库结论，再把任务记录复制到共享工具开发表。迁移记录只有在确实需要 Flyway 文件时才复制；最终迁移时间戳在合并顺序明确后批准，不要直接照抄开发者的候选时间戳。
